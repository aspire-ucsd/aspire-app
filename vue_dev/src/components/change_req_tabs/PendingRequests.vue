...
onBeforeMount(async () => {
    axios.get(`${contextData.tool_domain}/domain/changes/queue`).then((response) => {
...
const addVote = (id, index, vote) => {

    changeRequests.value.splice(index + currentIndex.value, 1)
    updateIndex(0)
    axios.put(`${contextData.tool_domain}/domain/changes/vote?request_id=${id}&vote=${vote}`).then((response) => {
        console.log(response)
    })
}

const confirmProposal = (changeRequest) => {
    changeRequest.comments.push({
        type: 'proposal', 
        content: `msg|A reviewer proposed the following change|data_type|entity_data|data|${JSON.stringify(changeRequest.entity_data)}`,
        submitted_by: Number(contextData.aspire_id), 
        submitted_at: Date.now()
    })

    const data = {type: 'proposal', content: `msg|A reviewer proposed the following change|data_type|entity_data|data|${JSON.stringify(changeRequest.entity_data)}`}

    axios.put(`${contextData.tool_domain}/domain/changes/comment?request_id=${changeRequest.id}`, data).then(() => {
        changeRequest.entity_data = updateParams.value.entity_data_copy
...
const closeOwnRequest = (id, index) => {
    axios.put(`${contextData.tool_domain}/domain/changes/close?change_request_id=${id}`).then(() => {
        changeRequests.value.splice(index + currentIndex.value, 1)
        updateIndex(0)
    })
}
    ...
    <div class="pending-main">
        ...
        <div class="pending-requests">
            ...
            >
                <div 
                    class="action-box"
                    v-if="updateParams.id !== changeRequest.id && changeRequest.submitted_by !== Number(contextData.aspire_id) && updateParams.id !== changeRequest.id"
                >
                    <button 
                        @click="() => addVote(changeRequest.id, index, 'approved')"
                        class="approve-button"
                    >
                        Approve
                    </button>
                    <button
                        v-if="changeRequest.modification_type !== 'delete'"
                        @click="
                        () => {
                    ...
                    <button 
                    @click="() => addVote(changeRequest.id, index, 'rejected')"
                    class="reject-button"
                    >
                        Reject
                    </button>
                ...
                <div 
                    v-if="changeRequest.submitted_by == Number(contextData.aspire_id) && !changeRequest.vote_type"
                    class="action-box"
                >
                    <button 
                        @click="() => closeOwnRequest(changeRequest.id, index)"
                        class="reject-button"
                    >
                        Close
                    </button>
                ...
                >
                    <button 
                        @click="() => confirmProposal(changeRequest)"
                        class="approve-button"
                    >
                        Confirm
                    </button>
